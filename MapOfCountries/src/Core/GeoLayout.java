...
public class GeoLayout implements Layout {
    ...
    public static String[] regions = {"No region","Africa","Americas","Asia","Europe","Oceania"};
    ...
    public void goAlgo() {
        try {
            ...
            for (AttributeColumn c : attModel.getNodeTable().getColumns()) {
                if (c.getId().equalsIgnoreCase("background_map_lat")) {
                    latitude = c;
                } else if (c.getId().equalsIgnoreCase("background_map_long")) {
                    longitude = c;
                }
            }
