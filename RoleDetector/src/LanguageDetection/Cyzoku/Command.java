...
public class Command {
     ...
     */
    private boolean loadProfile() throws IOException {
        String profileDirectory = get("directory") + "/";
        try {
            DetectorFactory.loadProfileChooseSource(profileDirectory, true);
     ...
     *
     */
    private void generateProfileFromText() throws LangDetectException {
        if (arglist.size() != 1) {
     ...
     *
     */
    public void detectLang() throws LangDetectException, IOException {
        if (loadProfile()) {
            return;
        }
        for (String filename : arglist) {
            ...
            try {
                ...
                detector.append(is);
//                System.out.53println(filename + ":" + detector.getProbabilities());
            } catch (IOException e) {
            } finally {
                try {
                    if (is != null) {
